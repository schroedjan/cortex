/**
 * opencode equivalent of .claude/hooks/user-prompt-submit.sh / pi's
 * cortex-route-visualizer.ts — detects requests that match the visualizer
 * row in the root AGENTS.md routing table and injects an explicit switch
 * directive as a synthetic message part, so mid-session routing doesn't
 * depend on the model noticing the match itself.
 *
 * Keep the keyword list in sync with that table's "Use when" column, with
 * .claude/hooks/user-prompt-submit.sh, .pi/extensions/cortex-route-visualizer.ts,
 * and the triggers in agents/visualizer/skills/{diagram,chart}/SKILL.md.
 */
...
export const CortexRouteVisualizer: Plugin = async () => {
  return {
    "chat.message": async (_input, output) => {
      const text = output.parts
        ...
        .join("\n");
      if (!VISUALIZER_TRIGGER.test(text)) return;
